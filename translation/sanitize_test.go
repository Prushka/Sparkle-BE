...
func TestSanitization(t *testing.T) {
	fBytes, err := os.ReadFile("test.ass")
	if err != nil {
	...
	if err := os.WriteFile("output_non_translate.ass", []byte(strings.Join(sub.nonTranslatableDialogues, "\n")), 0644); err != nil {
		t.Fatalf("Failed to write sanitized file: %v", err)
	}
...
func TestProcess(t *testing.T) {
	...
	inputsPairs := splitByCharacters(sub.distilledDialoguesWithIndex, 29000)
	text, err := sub.processIndex(inputsPairs[0], []string{
		"0,From my room,",
		"1,I can see the neighbor's backyard.",
		"2,It doesn't get a lot of sunlight...",
		"3,Rina-chan from next door is \na few years younger than I am,",
		"4,but apparently she's been sick all her life",
		"5,and never leaves the house.",
	})
	if err != nil {
